    ...
    var LogDecoder = function () {
        ...
        return {
            decode: function (evtData) {
                ...
                var msg = data.body || data;
                $ps.info('Attempting to decode JSON: ',msg);

                //var result = '';

                if (msg.total !== undefined) {
                    $("#filesProcessed").html(msg.total);
                    $("#filesInQueue").html(msg.queueSize || "0");
                }

                if (msg.successes !== undefined)
                    $("#filesSucceeded").html(msg.successes);

                if (msg.failures !== undefined)
                    $("#filesFailed").html(msg.failures);


                if (msg.lastDataflowsPk != undefined)
                    $('#lastFileId').html(msg.lastDataflowsPk);
