package click.rmx.engine.behaviour.ants;
...
import click.rmx.engine.behaviour.ai.StrategyEngine;
...
public class AntBehaviour extends SpriteBehaviour implements CollisionHandler, IKeyCallback {
	public static final String
	Possessed = AI_STATE_POSSESSED, //0
	...
	private float bounds = 400; //TODO: make this customizable
	...
	protected void onAwake() {
		...
		this.crowdStrategies.addMethod(Crowd.Turn_180, new Strategy.AiMethod() {
		...
		this.crowdStrategies.addMethod(Crowd.JumpUp, 10, new Strategy.AiMethod() {
		...
		this.crowdStrategies.addMethod(Crowd.Boost, 10, new Strategy.AiMethod(){
...
//				System.out.println("\nRESULTS:::");
	...
	void tryToAvoidCrowding(long tick) {
		...
		if (bumpCount == 0) {
			...
			if (ai != null && ai.state() == Strategy.State.WILL_CONTINUE) {
				ai.setState(Strategy.State.FINISHED);
