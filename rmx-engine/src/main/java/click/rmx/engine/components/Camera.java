...
import click.rmx.engine.math.BufferedMatrix4;
	...
	public void perspective(GLView view) {
        ...
        glFrustum( -fW, fW, -fH, fH, nearZ, farZ );
		 ...
		 BufferedMatrix4 m = new BufferedMatrix4 (
				 this.getNode().transform().worldMatrix()
		 );

		 m.setPosition(0,0,0);
		 m.invert();

	     glMultMatrixf(m.getBuffer());
	}
	
	public BufferedMatrix4 modelViewMatrix() {
		return new BufferedMatrix4 (
				this.getNode().transform().worldMatrix()
		);
	}
