package click.rmx.engine.behaviour.ai;
...
public class StrategyEngine extends HashMap<String, Strategy> {
	...
	public void addMethod(IMethods key, Strategy.AiMethod method) {
		this.addMethod(key, this.maxTurns, method);
	}
	
	public void addMethod(IMethods key, int maxTurns, Strategy.AiMethod method) {
...
//	Strategy current;
	...
	public Strategy invokeCurrent(Object... args) {
		...
		Strategy.State state = ai.invoke(args);
...
//		Bugger.logAndPrint(current().name + ": Success", false);
	...
	public void restartCurrent() {
		...
		this.current().setState(Strategy.State.WILL_CONTINUE);
	}
